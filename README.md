...
当任务需要视频、HTML、预览、转换、浏览器自动化或其他外部程序时，系统会先检查相关工具的当前版本和最新稳定兼容版本。缺少或过期的工具会优先在项目环境中自动安装或更新，然后运行诊断和最小预览。

视频路线会检查 Node.js 依赖、Remotion / HyperFrames、浏览器运行时和 FFmpeg；文章与 HTML 路线会检查当前选定的 `md2wechat`、`gzh-design`、Doocs/MD 或回退工具。网络下载和系统级安装必须经过 Codex 权限确认，不使用 `sudo`，也不会把未验证的安装当成成功。

如果更新会改变平台权限、发布行为或许可边界，流程会停止并交给人工审阅。纯定位、证据包和文字规划任务不会无意义地安装软件。详细规则见 [`UPDATE_POLICY.md`](UPDATE_POLICY.md)。
